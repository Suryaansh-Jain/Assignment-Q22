...
maximum. What is the price per unit and total profit at the level of production? \\
	...
	\begin{align}
	    \implies \frac{dy}{dx} &= 100 - \frac{x}{40} \\
	    \implies f'(x) &= 100 - \frac{x}{40} 
	\end{align}
	
	We will be able to find $x$ for maximum $f(x)$ by iterating the following equation till $f'(x_{k-1})$ approaches 0.
	
	\begin{align}
	    x_k = x_{k-1} - (\alpha f'(x_{k-1}))
	\end{align}
	
	When $f'(x_{k-1}) = 0$, we have
	
	\begin{align}
	    f'(x_{k-1}) &= 100 - \frac{x_{k-1}}{40} = 0 \\
	    \implies x_{k-1} &= 100.40 = 4000
	\end{align}
